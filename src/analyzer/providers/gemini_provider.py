"""Gemini-backed adapter for provider-independent transcript analysis."""
...
class GeminiModelsClient(Protocol):
    """Minimal injected Gemini model surface required by this provider."""

    def generate_content(self, **kwargs: object) -> object:
        """Generate one structured analysis response from supplied content."""


class GeminiClient(Protocol):
    """Minimal injected Gemini client surface required by this provider."""
...
class GeminiAnalysisProvider(AnalysisProvider):
    """Adapt an injected Gemini client to the common analysis-provider contract."""
    ...
    def __init__(
        ...
        Args:
            client: Authenticated Gemini client created by the composition root.
    ...
    def analyze(
        ...
        """Analyze a transcript using the injected Gemini client.
        ...
        try:
            response = self._client.models.generate_content(
                ...
                contents=self._build_provider_contents(request),
    ...
    def health_check(self) -> bool:
        """Return whether the injected client exposes the required model surface."""
        return callable(getattr(getattr(self._client, "models", None), "generate_content", None))

    def _build_provider_contents(self, request: AnalyzerRequest) -> str:
        """Build Gemini content from public transcript-analysis models."""
        payload = {
            "instruction": (
                "Identify high-retention short-form video moments. Return a JSON object with "
                "candidates and viral_moments. Preserve transcript timestamps exactly."
            ),
            "transcript": {
    ...
    def _parse_response(
        ...
        """Convert a Gemini response into the public analyzer result contract."""
    ...
    def _emit_progress(self, callback: ProgressCallback | None, progress: AnalysisProgress) -> None:
        ...
        try:
            ...
            self._logger.exception(
                "gemini_analysis_progress_callback_failed",
...
def _translate_provider_error(error: Exception) -> AnalyzerError:
    ...
    if status_code == 429 or any(keyword in message for keyword in ("quota", "rate limit")):
